...
{
    (void)fread(l.biases, sizeof(float), l.outputs, fp);
    (void)fread(l.weights, sizeof(float), l.outputs*l.inputs, fp);
    if(transpose){
    ...
    if (l.batch_normalize && (!l.dontloadscales)){
        (void)fread(l.scales, sizeof(float), l.outputs, fp);
        (void)fread(l.rolling_mean, sizeof(float), l.outputs, fp);
        (void)fread(l.rolling_variance, sizeof(float), l.outputs, fp);
...
{
    (void)fread(l.scales, sizeof(float), l.c, fp);
    (void)fread(l.rolling_mean, sizeof(float), l.c, fp);
    (void)fread(l.rolling_variance, sizeof(float), l.c, fp);
#ifdef GPU
    if(gpu_index >= 0){
...
{
    (void)fread(l.biases, sizeof(float), l.n, fp);
    if (l.batch_normalize && (!l.dontloadscales)){
        (void)fread(l.scales, sizeof(float), l.n, fp);
        (void)fread(l.rolling_mean, sizeof(float), l.n, fp);
        (void)fread(l.rolling_variance, sizeof(float), l.n, fp);
    }
    int size = l.c*l.size*l.size;
    int i, j, k;
    for(i = 0; i < l.n; ++i){
        float mean = 0;
        (void)fread(&mean, sizeof(float), 1, fp);
        for(j = 0; j < size/8; ++j){
            int index = i*size + j*8;
            unsigned char c = 0;
...
void load_convolutional_weights(layer l, FILE *fp)
{
    ...
    (void)fread(l.weights, sizeof(float), num, fp);
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    int minor;
    int revision;
    (void)fread(&major, sizeof(int), 1, fp);
    (void)fread(&minor, sizeof(int), 1, fp);
    (void)fread(&revision, sizeof(int), 1, fp);
    if ((major*10 + minor) >= 2 && major < 1000 && minor < 1000){
	uint64_t seen;
        (void)fread(&seen, sizeof(uint64_t), 1, fp);
	*net->seen = (size_t)seen;
    } else {
        int iseen = 0;
        (void)fread(&iseen, sizeof(int), 1, fp);
        *net->seen = iseen;
    }
    ...
    for(i = start; i < net->n && i < cutoff; ++i){
        ...
        if(l.type == LOCAL){
            ...
            int size = l.size*l.size*l.c*l.n*locations;
            (void)fread(l.biases, sizeof(float), l.outputs, fp);
            (void)fread(l.weights, sizeof(float), size, fp);
#ifdef GPU
            if(gpu_index >= 0){
                push_local_layer(l);
            }
